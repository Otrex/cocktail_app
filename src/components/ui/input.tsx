import * as React from "react";

import { cn } from "@/lib/utils";
import { SearchIcon } from "lucide-react";

function Input({
  className,
  wrapperClass,
  type,
  ...props
}: React.ComponentProps<"input"> & { wrapperClass?: string }) {
  const classNameCn = cn(
    "file:text-foreground placeholder:text-muted-foreground selection:bg-primary selection:text-primary-foreground dark:bg-input/30 border-input flex h-9 w-full min-w-0 rounded-md border bg-transparent px-3 py-1 text-base  transition-[color,box-shadow] outline-none file:inline-flex file:h-7 file:border-0 file:bg-transparent file:text-sm file:font-medium disabled:pointer-events-none disabled:cursor-not-allowed disabled:opacity-50 md:text-sm",
    "focus-visible:border-ring focus-visible:ring-ring/50 focus-visible:ring-[3px]",
    "aria-invalid:ring-destructive/20 dark:aria-invalid:ring-destructive/40 aria-invalid:border-destructive",
    className
  );

  if (type == "search") {
    return (
      <div
        className={cn(
          "inline-flex relative [&_input:focus-within~div>svg]:text-gray-700",
          wrapperClass
        )}
      >
        <input
          type={"text"}
          data-slot="input"
          className={cn(classNameCn, "pr-8")}
          {...props}
        />
        <div className="px-2 pr-4 absolute right-0 top-1/2 transform -translate-y-1/2 pointer-events-none">
          <SearchIcon
            strokeWidth="3"
            className=" size-4 text-muted-foreground"
          />
        </div>
      </div>
    );
  }
  return (
    <input type={type} data-slot="input" className={classNameCn} {...props} />
  );
}

export { Input };
